...
dvfs = zeros(sz(1),sz(2),2,1,'single');
for q = 1:14
   
 static = CB(:,:,17+q-1);
 p = get_canny_points( static );
    dfp = zeros(size(p,1),2); 
    moving = CB(:,:,17+q);
%     moving = move2dimage2(moving,-dvfs(:,:,1,q),-dvfs(:,:,2,q));
...
%%
%%
for i=1:15%:size(dvfs,4)
%     dvfs(:,:,:,q+1)
%    imagesc( CB(:,:,i));
   imagesc( dvfs(:,:,2,i),[-2,2]);
   colorbar
   drawnow
end
...
clc
dvfas = dvfs;
for qq = 1:size(dvfs,4)
%     q = index(qq);
%        dvfa = temp+dvfa;
        temp = zeros(sz(1),sz(2),2);
        for j = 1:2
%      temp(:,:,j)= move2dimage2(dvfs(:,:,j,q),-dvfs(:,:,1,q),-dvfs(:,:,2,q));
%        temp(:,:,j)= move2dimage2(dvfa(:,:,j),-dvfs(:,:,1,q),-dvfs(:,:,2,q));
       temp(:,:,j) = move2dimage2(dvfas(:,:,j,q),dvfas(:,:,1,q),dvfas(:,:,2,q));
        end
        dvfas(:,:,:,q+1) = dvfas(:,:,:,q+1)+temp;
end
dn = size(dvfas,4);
def = move2dimage2(CB(:,:,32),-dvfas(:,:,1,dn),-dvfas(:,:,2,dn));
size(dvfs,4)
subplot(1,3,1)
imagesc(dvfas(:,:,2,dn));
colorbar
...
% dvfa=dvfs(:,:,:,end);
% dvfa = zeros(sz(1),sz(2),2);
index = 1:size(dvfs,4);
index = flip(index,2);
        temp = zeros(sz(1),sz(2),2);
for qq = 1:size(dvfs,4)-1
    q = index(qq);
%        dvfa = temp+dvfa;
%        temp = zeros(sz(1),sz(2),2);
        for j = 1:2
...
def = move2dimage2(CB(:,:,32),-dvfs(:,:,1,q+1),-dvfs(:,:,2,q+1));
